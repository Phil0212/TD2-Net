...
class AG(Dataset):
    ...
    def __init__(self, mode, datasize, data_path=None, filter_nonperson_box_frame=True, filter_small_box=False):
        ...
        with open('/workspace/STMP-AR/data/rel_cb_weights_099999.pkl', 'rb') as f:
            self.class_weights = pickle.load(f)

        self.attention_weights = self.class_weights['attention_weight']
        self.spatial_weights = self.class_weights['spatial_weight']
        self.contact_weights = self.class_weights['contact_weight']
        ...
        if filter_small_box:
            ...
            f.close()
            with open(root_path + 'annotations/object_bbox_and_relationship_filtersmall.pkl', 'rb') as f:
