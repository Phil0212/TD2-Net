...
class BasicSceneGraphEvaluator_MRecall:
    def __init__(self, mode, AG_object_classes, AG_all_predicates, AG_attention_predicates, AG_spatial_predicates,
        ...
        self.recall_count = [0] * self.num_rel

        self.result_dict[mode + '_mean_micro_recall'] = {10: 0.0, 20: 0.0, 50: 0.0}
        self.result_dict[mode + '_mean_micro_recall_collect_hit'] = {10: [0.0 for i in range(self.num_rel)],
                                                                     20: [0.0 for i in range(self.num_rel)],
                                                                     50: [0.0 for i in range(self.num_rel)]}
        self.result_dict[mode + '_mean_micro_recall_collect_gt'] = {10: [0.0 for i in range(self.num_rel)],
                                                                    20: [0.0 for i in range(self.num_rel)],
                                                                    50: [0.0 for i in range(self.num_rel)]}
        self.result_dict[mode + '_mean_micro_recall_list'] = {10: [], 20: [], 50: []}

    def reset_result(self):
        # self.result_dict[self.mode + '_mean_recall'] = {10: 0.0, 20: 0.0, 50: 0.0}
        self.result_dict[self.mode + '_mean_micro_recall'] = {10: 0.0, 20: 0.0, 50: 0.0}
        self.result_dict[self.mode + '_mean_micro_recall_collect_hit'] = {10: [0.0 for i in range(self.num_rel)],
                                                                          20: [0.0 for i in range(self.num_rel)],
                                                                          50: [0.0 for i in range(self.num_rel)]}
        self.result_dict[self.mode + '_mean_micro_recall_collect_gt'] = {10: [0.0 for i in range(self.num_rel)],
                                                                         20: [0.0 for i in range(self.num_rel)],
                                                                         50: [0.0 for i in range(self.num_rel)]}
        self.result_dict[self.mode + '_mean_micro_recall_list'] = {10: [], 20: [], 50: []}
    ...
    def calculate_mean_recall(self, mode):
        for k, v in self.result_dict[mode + '_mean_micro_recall'].items():
            ...
            for idx in range(num_rel_no_bg):
                if self.result_dict[mode + '_mean_micro_recall_collect_hit'][k][idx] == 0.0:
                    tmp_recall = 0.0
                else:
                    tmp_recall = self.result_dict[mode + '_mean_micro_recall_collect_hit'][k][idx] / \
                                 self.result_dict[mode + '_mean_micro_recall_collect_gt'][k][idx]
    ...
    def print_stats(self):
        self.calculate_mean_recall(self.mode)
        print('======================' + self.mode + '============================')
        for k, v in self.result_dict[self.mode + '_mean_micro_recall'].items():
            print('MR@%i: %f' % (k, v))
...
def evaluate_from_dict(gt_entry, pred_entry, mode, result_dict, method=None, threshold=0.9, num_rel=26, **kwargs):
    ...
    for k in result_dict[mode + '_mean_micro_recall_collect_hit']:
        ...
        # recall_hit = result_dict[mode + '_mean_micro_recall_collect_hit'][k]
        # recall_count = result_dict[mode + '_mean_micro_recall_collect_gt'][k]
        for idx in range(gt_rels.shape[0]):
            local_label = gt_rels[idx, 2]
            result_dict[mode + '_mean_micro_recall_collect_gt'][k][int(local_label)] += 1
            

        for idx in range(len(match)):
            local_label = gt_rels[int(match[idx]), 2]
            result_dict[mode + '_mean_micro_recall_collect_hit'][k][int(local_label)] += 1
